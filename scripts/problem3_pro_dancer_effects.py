...
def main():
    ...
    x = np.arange(10)
    plt.bar(x - 0.2, dancer_coef_j["coef"].values, width=0.4, label="Judge Score Effect")
    plt.bar(x + 0.2, dancer_coef_f["coef"].values, width=0.4, label="Audience Vote Effect")
    plt.xticks(x, [s.replace(dancer_prefix, "") for s in dancer_coef_j["feature"].values], rotation=45, ha="right")
    plt.title("Pro Dancer Effects (Top 10): Differences between Judges and Audience")
    plt.ylabel("Regression Coefficient (Relative to Baseline)")
    plt.xlabel("Professional Dancer")
